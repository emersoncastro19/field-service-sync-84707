...
export default function CoordinadorDetallesOrden() {
  ...
  useEffect(() => {
    if (!idOrden) {
      ...
      navigate('/coordinador/historial');
      return;
    }
                ...
                <CardContent className="space-y-4">
                  {orden.ejecuciones.map((ejecucion) => (
                    <div key={ejecucion.id_ejecucion} className="p-4 border rounded-lg space-y-3">
                      ...
                      <div>
                        <p className="text-sm text-muted-foreground mb-2">Confirmación del Cliente</p>
